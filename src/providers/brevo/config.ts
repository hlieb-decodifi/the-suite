...
async function getBrevoTemplatePreview(templateId: number, params: Record<string, unknown>): Promise<TemplateContent | null> {
  try {
    ...
    const previewResponse = await fetch('https://api.brevo.com/v3/smtp/template/preview', {
      ...
      body: JSON.stringify({ templateId, params, email: 'hlieb@decodifi.uk' }),
    });

    if (!previewResponse.ok) {
      const errorText = await previewResponse.text();
      console.error(`Failed to get template preview: ${previewResponse.statusText}`, errorText);
      
      // Fallback to template content without variable substitution
      console.log('Falling back to template content without variable substitution');
      return {
    ...
    return {
      subject: preview.subject || template.subject || template.name || `Template ${templateId}`,
      html: preview.html || template.htmlContent || '<p>Template content not available</p>',
    };
  } catch (error) {
    ...
    if (error instanceof Error && error.message.includes('404')) {
      console.error(`Template ${templateId} does not exist in your Brevo account. Please check the template ID.`);
    }
