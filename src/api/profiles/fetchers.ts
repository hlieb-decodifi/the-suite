...
import { Address, AddressFormData, ClientProfile, DetailFormData } from './types';
...
export async function getClientAddress(addressId: string | null): Promise<Address | null> {
...
export async function getClientProfileWithAddress(userId: string): Promise<{
  ...
  const address = profile?.address_id ? await getClientAddress(profile.address_id) : null;
  
  return { profile, address };
}

// Update user details
export async function updateUserDetails(user: User, details: DetailFormData): Promise<boolean> {
  const supabase = createClient();
...
export async function createAddress(addressData: AddressFormData): Promise<string | null> {
  const supabase = createClient();
...
export async function updateAddress(addressId: string, addressData: AddressFormData): Promise<boolean> {
  const supabase = createClient();
