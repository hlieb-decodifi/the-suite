...
export function useActivityTracker() {
  ...
  const trackServiceView = useCallback(
    (serviceId: string, metadata?: Record<string, unknown>) => {
      return track(
        ActivityTypeEnum.SERVICE_VIEW,
  ...
  const trackProfessionalView = useCallback(
    (professionalId: string, metadata?: Record<string, unknown>) => {
      return track(
        ActivityTypeEnum.PROFESSIONAL_VIEW,
        EntityTypeEnum.PROFESSIONAL,
        professionalId,
        metadata,
      );
    },
  ...
  const trackBookingStarted = useCallback(
    (serviceId: string, metadata?: Record<string, unknown>) => {
      return track(
        ActivityTypeEnum.BOOKING_STARTED,
        EntityTypeEnum.SERVICE,
        serviceId,
        metadata,
      );
  ...
  const trackBookingCompleted = useCallback(
    (bookingId: string, metadata?: Record<string, unknown>) => {
      return track(
        ActivityTypeEnum.BOOKING_COMPLETED,
  ...
  const trackBookingCancelled = useCallback(
    (bookingId: string, metadata?: Record<string, unknown>) => {
      return track(
        ActivityTypeEnum.BOOKING_CANCELLED,
        EntityTypeEnum.BOOKING,
        bookingId,
        metadata,
      );
  ...
  const trackSearch = useCallback(
    (searchTerm: string, metadata?: Record<string, unknown>) => {
