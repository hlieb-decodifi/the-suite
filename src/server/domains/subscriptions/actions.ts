...
export async function syncStripePriceWithDatabase(
  price: Stripe.Price,
): Promise<boolean> {
...
export async function getSubscriptionPlansWithVerifiedPrices(): Promise<
  SubscriptionPlan[]
> {
  try {
    ...
    const plansWithStripeIds = plans.filter(
      (plan) => plan.stripePriceId && plan.stripePriceId.length > 0,
    );
...
export async function createCheckoutSession(
  ...
  if (authUser.id !== userId) {
    throw new Error(
      'Unauthorized: You can only subscribe for your own account',
    );
  }
...
export async function cancelSubscription(
  ...
  try {
    ...
    if (authUser.id !== userId) {
      throw new Error(
        'Unauthorized: You can only manage your own subscription',
      );
    }
    ...
    if (existingConnectData?.professional_stripe_connect?.stripe_account_id) {
      try {
        account = await stripe.accounts.retrieve(
          existingConnectData.professional_stripe_connect.stripe_account_id,
        );
        console.log('Retrieved existing Stripe account from DB:', account.id);
      } catch (error) {
        console.log(
          'Stored account ID invalid, will create new account:',
          error,
        );
    ...
    if (dbStatus) {
      ...
      if (profileData?.professional_stripe_connect?.stripe_connect_updated_at) {
        const lastUpdate = new Date(
          profileData.professional_stripe_connect.stripe_connect_updated_at,
        );
...
async function getStripeConnectStatusFromAPI(userId: string): Promise<{
  ...
  try {
    ...
    if (authUser.id !== userId) {
      throw new Error(
        'Unauthorized: You can only check your own Stripe Connect status',
      );
    }
