 ...
 */
export async function getPortfolioPhotosForUser(
  userId: string,
 ...
 */
export async function getPortfolioPhotoById(
  id: string,
): Promise<PortfolioPhotoDB | null> {
  const supabase = await createClient();
...
export async function updatePortfolioPhoto(
  ...
  if (fetchError || !photo) {
    throw new Error(
      'Photo not found or you do not have permission to update it',
    );
  }
