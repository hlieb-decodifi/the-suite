...
export async function handleDualPaymentCancellation(
  ...
  console.log(`[Dual Payment Cancellation] Starting cancellation for dual payment structure`);
  ...
  try {
    ...
    if (professionalProfile && professionalProfile.cancellation_policy_enabled && (!isProfessional || forcePolicy)) {
      console.log(`[Policy] Checking cancellation policy - Hours until appointment: ${hoursUntilAppointment.toFixed(1)}`);
      
      if (hoursUntilAppointment < 24) {
        chargePercentage = professionalProfile.cancellation_24h_charge_percentage || 50;
        isWithinPolicyPeriod = true;
      } else if (hoursUntilAppointment < 48) {
        chargePercentage = professionalProfile.cancellation_48h_charge_percentage || 25;
        isWithinPolicyPeriod = true;
      }
    ...
    const totalCancellationFee = Math.round(servicePriceAmount * (chargePercentage / 100) * 100); // Convert to cents
    ...
    if (isWithinPolicyPeriod && chargePercentage > 0) {
      ...
      console.log(`[Policy Cancellation] Applying cancellation policy: ${chargePercentage}% fee`);
      ...
      console.log(`[No Policy] No cancellation policy applies - full refund/cancellation`);
    ...
    console.error('[Dual Payment Cancellation] ❌ Error during cancellation:', error);
    return {
      depositRefunded,
      balanceCancelled,
      chargeAmount,
      error: error instanceof Error ? error.message : 'Unknown error during cancellation'
    };
  }
}
...
async function handlePolicyCancellation(
  ...
  const servicePriceInCents = (payment.amount + (payment.tip_amount || 0)) * 100; // Service price + tip in cents
  ...
  if (depositAmount > 0) {
    ...
    depositCancellationFee = Math.round((depositAmount / servicePriceInCents) * totalCancellationFeeInCents);
    balanceCancellationFee = totalCancellationFeeInCents - depositCancellationFee;
  } else {
  ...
  if (payment.deposit_payment_intent_id && depositAmount > 0) {
    console.log(`[Policy Deposit] Processing deposit: ${payment.deposit_payment_intent_id}`);
    ...
    try {
      ...
      if (depositPaymentIntent.status === 'succeeded') {
        ...
        if (depositRefundAmount > 0) {
          ...
          console.log(`[Policy Deposit] ✅ Partial deposit refund: $${depositRefund.amount / 100} (kept $${depositCancellationFee / 100} as cancellation fee)`);
        } else {
          console.log(`[Policy Deposit] ✅ No deposit refund - full amount kept as cancellation fee`);
        }
        ...
        if (depositCancellationFee > 0) {
          const captureResult = await stripe.paymentIntents.capture(payment.deposit_payment_intent_id, {
            amount_to_capture: depositCancellationFee
          });
          console.log(`[Policy Deposit] ✅ Partial deposit capture: $${captureResult.amount_received / 100} as cancellation fee`);
        } else {
          // Cancel if no fee to capture
          await stripe.paymentIntents.cancel(payment.deposit_payment_intent_id);
          console.log(`[Policy Deposit] ✅ Cancelled uncaptured deposit (no fee)`);
        }
      }
    } catch (error) {
      console.error('[Policy Deposit] ❌ Failed to handle deposit:', error);
      throw new Error(`Failed to handle deposit: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }

  // Step 2: Handle balance - capture cancellation fee from balance portion, always return suite fee
  if (payment.stripe_payment_intent_id) {
    console.log(`[Policy Balance] Processing balance payment: ${payment.stripe_payment_intent_id}`);
    
    try {
      const balancePaymentIntent = await stripe.paymentIntents.retrieve(payment.stripe_payment_intent_id);
      ...
      if (balancePaymentIntent.status === 'requires_capture') {
        ...
        if (totalCaptureAmount > 0 && totalCaptureAmount < balanceAmount) {
          try {
            // Partial capture: capture only the cancellation fee
            const captureResult = await stripe.paymentIntents.capture(payment.stripe_payment_intent_id, {
              amount_to_capture: totalCaptureAmount
            });
            
            console.log(`[Policy Balance] ✅ Partial capture successful: $${captureResult.amount_received / 100} (client gets back $${(balanceAmount - totalCaptureAmount) / 100})`);
            
          } catch (captureError) {
            console.log(`[Policy Balance] Partial capture failed, using dual charge approach:`, captureError);
            
            // Fallback: Cancel uncaptured payment and create separate charge for cancellation fee
            await stripe.paymentIntents.cancel(payment.stripe_payment_intent_id);
            console.log(`[Policy Balance] ✅ Cancelled uncaptured payment intent`);
          ...
          const captureResult = await stripe.paymentIntents.capture(payment.stripe_payment_intent_id);
          console.log(`[Policy Balance] ✅ Full capture: $${captureResult.amount_received / 100} (cancellation fee >= balance)`);
        } else {
          // No fee to capture from balance, cancel the payment
          await stripe.paymentIntents.cancel(payment.stripe_payment_intent_id);
          console.log(`[Policy Balance] ✅ Cancelled uncaptured payment (no balance cancellation fee)`);
        }
        ...
        if (refundAmount > 0) {
          ...
          console.log(`[Policy Balance] ✅ Partial refund processed: $${refund.amount / 100} (kept $${balanceCancellationFee / 100} as cancellation fee)`);
        } else {
          console.log(`[Policy Balance] ✅ No refund - full balance kept as cancellation fee`);
        }
      } else {
        console.log(`[Policy Balance] Payment in status ${balancePaymentIntent.status} - no action needed`);
      }
      
    } catch (error) {
      console.error('[Policy Balance] ❌ Failed to handle balance payment:', error);
      throw new Error(`Failed to handle balance payment: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
}
...
async function handleNoPolicyCancellation(
  ...
  if (payment.deposit_payment_intent_id && (payment.deposit_amount || 0) > 0) {
    console.log(`[No Policy Deposit] Processing deposit refund: ${payment.deposit_payment_intent_id}`);
    ...
    try {
      ...
      if (depositPaymentIntent.status === 'succeeded') {
        ...
        console.log(`[No Policy Deposit] ✅ Deposit refunded: $${depositRefund.amount / 100}`);
      } else if (depositPaymentIntent.status === 'requires_capture') {
  ...
  if (payment.stripe_payment_intent_id) {
    console.log(`[No Policy Balance] Processing balance payment: ${payment.stripe_payment_intent_id}`);
    ...
    try {
      ...
      if (balancePaymentIntent.status === 'requires_capture') {
        ...
        await stripe.paymentIntents.cancel(payment.stripe_payment_intent_id);
        console.log(`[No Policy Balance] ✅ Uncaptured balance payment cancelled`);
        
      } else if (balancePaymentIntent.status === 'succeeded') {
        // Refund the balance payment (minus service fee for client cancellations)
        const serviceFee = isProfessional ? 0 : (payment.service_fee || 0) * 100; // Convert to cents
        ...
        if (refundAmount > 0) {
          ...
          console.log(`[No Policy Balance] ✅ Balance refunded: $${refund.amount / 100}`);
        }
      } else {
        console.log(`[No Policy Balance] Payment in status ${balancePaymentIntent.status} - no action needed`);
      }
      
    } catch (error) {
      console.error('[No Policy Balance] ❌ Failed to handle balance payment:', error);
      throw new Error(`Failed to handle balance payment: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
...
async function createSeparateCancellationCharge(
  ...
  console.log(`[Separate Charge] Creating separate cancellation charge: $${chargeAmountInCents / 100}`);
  
  try {
    // Get customer ID and payment method
    const { getStripeCustomerId } = await import('../stripe-payments/db');
    const customerId = await getStripeCustomerId(userId);
    const professionalStripeAccount = professionalProfile?.professional_stripe_connect?.stripe_account_id || null;
    ...
    if (!payment.stripe_payment_method_id) {
      throw new Error('Payment method not found - cannot create separate charge');
    }
    ...
    console.log(`[Separate Charge] ✅ Cancellation charge created: ${charge.id} - $${charge.amount / 100}`);
    
  } catch (error) {
    console.error('[Separate Charge] ❌ Failed to create separate cancellation charge:', error);
    // Don't throw here - cancellation should still proceed even if fee charge fails
    console.log(`[Separate Charge] ⚠️ Continuing with cancellation despite charge failure`);
  }
}
