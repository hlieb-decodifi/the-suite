 ...
 */
export async function getAvailablePaymentMethodsFromDb(): Promise<
  PaymentMethod[]
> {
 ...
 */
export async function getProfessionalProfileIdReadOnly(
  userId: string,
): Promise<string | null> {
  const supabase = await createClient();
  ...
  if (error || !data) {
    ...
    if (
      ...
      console.error(
        'Error fetching professional profile ID (read-only):',
        error,
      );
    }
    return null;
 ...
 */
export async function getProfessionalProfileId(
  userId: string,
): Promise<string> {
  const supabase = await createClient();
 ...
 */
export async function getProfessionalPaymentMethodsFromDbReadOnly(
  userId: string,
  ...
  try {
    ...
    const professionalProfileId =
      await getProfessionalProfileIdReadOnly(userId);
    ...
    if (error) {
      ...
      if (
        ...
        console.error(
          'Error fetching professional payment methods (read-only):',
          error,
        );
      }
 ...
 */
export async function getProfessionalPaymentMethodsFromDb(
  userId: string,
): Promise<PaymentMethod[]> {
  const supabase = await createClient();
  ...
  try {
    ...
    if (
      ...
      error.message.includes('Professional profile not found')
    ) {
...
export async function updateProfessionalPaymentMethodsInDb(
  ...
  try {
    ...
    const methodsToAdd = selectedMethodIds.filter(
      (id) => !currentMethodIds.includes(id),
    );
    const methodsToDelete = currentMethodIds.filter(
      (id) => !selectedMethodIds.includes(id),
    );
