  ...
  try {
    ...
    if (isPublished && !hasEnabledHours) {
      return {
        success: false,
        error:
          'Cannot remove all working hours while profile is published. You must have at least one working day selected.',
      };
    }
    ...
    const message =
      err instanceof Error ? err.message : 'Failed to update working hours';
...
export async function getProfessionalTimezoneAction(userId: string): Promise<{
  ...
  try {
    ...
    const message =
      err instanceof Error ? err.message : 'Failed to fetch timezone';
    return { success: false, error: message };
  }
}
