...
export function useChangeEmailForm({ 
  ...
  const handleSubmit = useCallback(async (data: ChangeEmailFormValues) => {
    try {
      setIsPending(true);
      
      // Call the server action for email change
      const result = await updateEmailAction(data.newEmail, data.currentPassword);
      ...
      toast({
        ...
        description: "Please check your current email address to confirm the change."
      });
