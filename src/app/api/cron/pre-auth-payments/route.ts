...
export async function GET() {
  ...
  try {
    ...
    console.log(`[CRON] Found ${pendingPayments.length} payments pending pre-auth`);
    ...
    for (const payment of pendingPayments) {
      try {
        console.log(`[CRON] Processing pre-auth for booking: ${payment.booking_id}`);
        ...
        processedCount++;
        console.log(`[CRON] Successfully pre-authorized payment for booking: ${payment.booking_id}`);

      } catch (error) {
        errorCount++;
        const errorMessage = `Booking ${payment.booking_id}: ${error instanceof Error ? error.message : 'Unknown error'}`;
        errors.push(errorMessage);
        console.error(`[CRON] Error processing pre-auth for booking ${payment.booking_id}:`, error);
      }
    }

    const duration = Date.now() - startTime;
    
    console.log(`[CRON] Pre-auth processing completed. Processed: ${processedCount}, Errors: ${errorCount}, Duration: ${duration}ms`);
