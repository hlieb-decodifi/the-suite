...
export function getTimezoneOptions() {
  ...
  const timezonesWithGMT = timezones
    ...
    .filter(
      (timezone): timezone is NonNullable<typeof timezone> => timezone !== null,
    );

  // Remove duplicates by value (in case any accidentally got added)
  const uniqueTimezones = timezonesWithGMT.filter(
    (timezone, index, self) =>
...
export function convertTimeToClientTimezone(
  ...
  try {
    ...
    const timeInUTC = fromZonedTime(
      professionalLocalTime,
    ...
    const dayOffset = Math.round(
      (clientDate.getTime() - professionalDate.getTime()) /
        (24 * 60 * 60 * 1000),
    );
...
export function convertWorkingHoursToClientTimezone(
  ...
  professionalHours.hours.forEach((dayHours) => {
    ...
    if (startConversion.dayOffset !== endConversion.dayOffset) {
      ...
      const startDayIndex =
        (originalDayIndex + startConversion.dayOffset + 7) % 7;
      ...
      if (startDayName) {
        ...
        const startDayResultIndex = resultHours.findIndex(
          (h) => h.day === startDayName,
        );
      ...
      if (endDayName) {
        ...
        const endDayResultIndex = resultHours.findIndex(
          (h) => h.day === endDayName,
        );
      ...
      if (targetDayName) {
        const targetDayResultIndex = resultHours.findIndex(
          (h) => h.day === targetDayName,
        );
...
export function getAvailableDaysWithTimezoneConversion(
  ...
  if (professionalHours.timezone === clientTimezone) {
    return professionalHours.hours
      .filter(
        (dayHours) =>
          dayHours.enabled && dayHours.startTime && dayHours.endTime,
      )
  ...
  professionalHours.hours.forEach((dayHours) => {
    ...
    try {
      ...
      const professionalStartTime = fromZonedTime(
        new Date(testStartTime),
        professionalHours.timezone,
      );
      const professionalEndTime = fromZonedTime(
        new Date(testEndTime),
        professionalHours.timezone,
      );

      // Convert to client timezone
      const clientStartTime = toZonedTime(
      ...
      console.error(
        'Error converting working hours for day:',
        dayHours.day,
        error,
      );
    }
...
export function parseWorkingHoursFromDB(
  ...
  if (Array.isArray(workingHoursJSON)) {
    return migrateWorkingHoursToTimezoneAware(
      workingHoursJSON,
      fallbackTimezone,
    );
  }
